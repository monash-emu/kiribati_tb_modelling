...
\title{Using early screening data to guide population-wide tuberculosis screening in Kiribati: a transmission modelling study\\[0.6em]
\large Methods appendix}
\date{}
\author{Romain Ragonnet, Jeremy Hill, Alfred Tonganibeia, David Shipman, Kerrie Shaw,\\
Emma S McBryde, Warwick Britton, Ben J Marais, James M Trauer\\[0.6em]}
...
The contained stage represents a state in which infection is controlled by the immune system, with no immediate risk of progression to active disease. 
...
Containment also allows for complete clearance of infection, represented by state \(X\).
...
The model is simulated from 1850 to 2035. The initial population size is set to the earliest available estimate for the modelled population (1950) from the United Nations World Population Prospects, and no population growth is applied before that first data year, such that the population size remains constant over the initial period and follows the observed trajectory thereafter (Section~\ref{sec:demog}).
The epidemic is seeded at the start of the simulation with 100 individuals in the symptomatic, high-infectiousness disease state (\(D_{SH}\)), with the remainder of the population assigned to the M.tb-na\"ive compartment (\(S\)).
...
Parameters with specified prior distributions are estimated through our Bayesian calibration algorithm while those without priors are fixed. 
...
The population is stratified by age, and every age and disease state in the model is subject to the applicable all‑cause and TB‑specific mortality rates. Age‑specific all-cause (non-TB) mortality rates and the total population size over time are taken from the United Nations World Population Prospects, as described in Section~\ref{sec:demog-inputs}.
...
Let \(i,j \in \{1,\dots,G\}\) index age groups and $a_{i}$, $b_{i}$ denote the lower and upper bounds of group \(i\). Calendar time is \(t\) (years). 
The construction uses the United Nations World Population Prospects population and fertility series described in Section~\ref{sec:demog-inputs}; for modelled years outside the data ranges, quantities are clamped to the nearest available year.
\paragraph{Overview.}
...
\]
where \(b\) is the background level, \(S^{\mathrm{assort}}\) is an exponential kernel with spread \(\ell\), and \(S^{\mathrm{pc}}\) encodes parent–child contacts from fertility patterns, with strength \(\kappa_{\mathrm{pc}}\). Smaller values of \(\ell\) correspond to more strongly assortative mixing by age.
...
\begin{itemize}
    \item \textbf{Susceptibility to infection.} All infection and reinfection flows (from \(S\), \(C\), \(X\) and \(R\)) are multiplied by \(\theta_{\mathrm{sus}}\) in the hard-to-reach stratum, so that hard-to-reach individuals acquire infection at a higher rate than screening-reachable individuals experiencing the same force of infection. This factor is applied to the susceptibility side of transmission only; the relative infectiousness of individuals with TB does not differ by stratum.
    \item \textbf{Passive detection.} The time-varying passive detection rate \(\delta(t)\) (Section~\ref{sec:screening}) is multiplied by \(\theta_{\mathrm{det}}\) in the hard-to-reach stratum, reflecting an assumption of reduced access to routine diagnostic services in the same population that is difficult to reach through active screening.
\end{itemize}
...
\paragraph{Rationale for the transmission-scaling exponent.}
The exponent \(\kappa\) determines how the risk to a susceptible individual responds to a change in the size of the population with which they mix. At \(\kappa=1\) transmission is purely frequency-dependent: the force of infection depends on the \emph{prevalence} of infectious disease, \(I^{\mathrm{eff}}_j/N_j\), so that a doubling of the population accompanied by a doubling of the number of infectious individuals leaves individual risk unchanged. At \(\kappa=0\) transmission is purely density-dependent: the force of infection depends on the \emph{number} of infectious individuals, \(I^{\mathrm{eff}}_j\), so that the same doubling doubles individual risk. Intermediate values interpolate between these two regimes, with smaller values implying that population growth translates more completely into increased transmission potential.

Neither limiting case is clearly appropriate for the modelled setting, and the choice is consequential here because the population of South Tarawa has grown several-fold over the simulated period and is projected to continue growing. Frequency dependence is the conventional assumption for respiratory pathogens, on the grounds that the number of close contacts an individual makes is governed by social and behavioural constraints rather than by the size of the surrounding population, and that contact rates therefore saturate as a population grows. That argument is weaker where growth occurs within a fixed and severely constrained habitable area, as it does in South Tarawa: the land available for settlement cannot expand, so a larger population is accommodated through greater crowding---larger households, more occupants per dwelling and per room, and more intense sharing of indoor space---rather than through spatial expansion. Since M.tb transmission is concentrated in shared, poorly ventilated indoor environments, crowding of this kind is expected to increase the number and the duration of effective exposures per person, which corresponds to some degree of density dependence.

No observation available for this analysis identifies where between the two regimes the setting lies, and the quantity is not separately measurable from the notification and prevalence data, which are informative about the level of transmission but only weakly about its scaling with population size. Rather than imposing either extreme, \(\kappa\) is therefore treated as a free parameter with a uniform prior over its full admissible range \([0,1]\) and estimated jointly with the other transmission parameters, so that the uncertainty in this structural assumption is carried explicitly through the calibration and propagated into the projections. The scaling constant \(N_{\mathrm{ref}}\) introduced above keeps \(\beta_0\) on a comparable scale across the range of \(\kappa\), so that the prior on \(\beta_0\) does not implicitly favour particular values of \(\kappa\) (Section~\ref{sec:popscaling}).
...
\begin{itemize}
    \item \textbf{Symptom screening (SSx)}: screening on the basis of reported symptoms alone. Under this approach, symptomatic disease is assumed to be identified with certainty and asymptomatic disease not at all, so that this approach cannot detect asymptomatic TB by construction.
    ...
    \item \textbf{Xpert+CXR}: symptom screening and CXR, with Xpert MTB/RIF Ultra additionally used as a frontline test. This is the full algorithm used in the PEARL study, and it defines what counts as active TB in the model. That is, the disease states are populated by what this algorithm detects, such that its sensitivity is one in all four states by construction (Section~\ref{sec:params}).
\end{itemize}
...
Screening for non-disease M.tb infection is performed using the TST, applied to the incipient and contained states with a common probability of a positive result for both of these states.

TST reactivity is not restricted to these two states. Individuals who have cleared infection or recovered from disease may also react to the test, and individuals with active TB are assumed to react with probability one. These states are therefore included when computing the modelled TST positivity used as a calibration target, but no preventive treatment flow originates from them: individuals who have cleared or recovered are not at risk of progression and so derive no benefit from TPT, while those with active disease would be identified and treated through the concurrent disease-screening arm of the same campaign.
...
Let \(\alpha_i(t)\) be the ageing rate from group \(i\) to \(i{+}1\) (\(\alpha_G=0\)), and \(\mu_i(t)\) the all-cause mortality rate; neither of which depends on \(s\). For any compartment variable \(U\in\{S,E,C,X,D_{AL},D_{AH},D_{SL},D_{SH},T,R\}\), define the (within-stratum) ageing operator
\[
...
The three multiplicative adjustments that distinguish the two strata (Section~\ref{sec:reachability}) are written as the following stratum-indexed factors:
\[
\theta^{\mathrm{sus}}_s =
\begin{cases}
1, & s=\mathrm{r},\\
...
\end{cases}
\]
so that the 95\% interval of the likelihood spans \(\pm 20\%\) (or \(\pm 40\%\) for notifications) of the mean observed value. 

The wider tolerance applied to the notification target reflects the weaker correspondence between this observation and the quantity the model produces. The model output is the number of individuals commencing TB treatment, whereas the notification series is a programmatic record compiled over several decades, subject to incomplete and variable reporting, changes in recording practice and diagnostic capacity over time, and small annual counts that fluctuate substantially for reasons unrelated to the underlying epidemic. It was additionally necessary to impute the locality of a proportion of notifications each year in order to restrict the series to the modelled area (Section~\ref{sec:calib-data}), which introduces further error. The remaining targets were measured directly during the completed PEARL screening phase under a defined protocol and correspond more closely to their modelled counterparts. Widening the notification tolerance therefore prevents the long annual series from dominating the likelihood relative to these cross-sectional measurements, and allows the calibrated trajectory to reproduce the level and shape of the notification series without being drawn towards year-to-year variation that the model is not intended to capture.
...
The estimated mixing matrix is compared with the external synthetic matrix using the Canberra distance between the spectral-radius-normalised model matrix \(\tilde{\mathbf{C}}(t)\) evaluated in 2025 and the normalised external matrix \(\mathbf{C}^{\mathrm{ext}}\):
\[
d(\theta)
=
\sum_{i,j}
...
We performed calibration using Markov chain Monte Carlo (MCMC) sampling with the \texttt{DEMetropolisZ} algorithm implemented in \texttt{PyMC}. We ran eight parallel chains to improve exploration of the posterior distribution. Each chain used 10,000 tuning (adaptation) iterations, followed by 20,000 posterior draws, giving 240,000 model evaluations per calibration. For the subsequent scenario projections, we discard an initial burn-in period of 10,000 draws and retain 2,000 posterior samples per calibrated run.
...
Each model configuration was calibrated independently, and the configurations were run in parallel as array jobs on a high-performance computing cluster, each using eight cores. The full analysis required between approximately 10 and 15 hours, depending on the parameter values explored.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
